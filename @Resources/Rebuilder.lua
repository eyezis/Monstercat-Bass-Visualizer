...
function Rebuild()
	...
	local iBarCount = RmGetUInt("BarCount", 200)
	if iBarCount <= 0 then iBarCount = 200 end

	local iBandCount = RmGetUInt("BandCount", 40)
	if iBandCount <= 0 then iBandCount = 40 end

	oMeters = iniBuilder()
	oMeasures = iniBuilder()

	local o = oMeasures:NewSection("MeasureAudio")
		o:AddKey("Measure", "Plugin")
		o:AddKey("Plugin", "AudioLevel")
		o:AddKey("Port", "Output")
		o:AddKey("FFTSize", 16384)
		o:AddKey("FFTOverlap", 10648)
		o:AddKey("FFTAttack", 64)
		o:AddKey("FFTDecay", 32)
		o:AddKey("FreqMin", 30)
		o:AddKey("FreqMax", 130)
		o:AddKey("Sensitivity", 20)
		o:AddKey("Bands", iBandCount + 1)
	o:Commit()

	for i=1,iBandCount do
		local o = oMeasures:NewSection("MsBand" .. i)
			o:AddKey("Measure", "Plugin")
			o:AddKey("Plugin", "AudioLevel")
	...
	for i=1,iBarCount do
		local o = oMeters:NewSection("MtBar" .. i)
			o:AddKey("Meter", "Bar")
			o:AddKey("MeterStyle", "StBar")
			o:AddKey("MeasureName", "MsCalc" .. i)
			o:AddKey("X", ("(%s * #BarTotalWidth#)"):format(i - 1))
		o:Commit()

		local o = oMeasures:NewSection("MsCalc" .. i)
			o:AddKey("Measure", "Calc")
			-- TODO:
			if i <= iBandCount then
				o:AddKey("Formula", "MsBand" .. i)
			else
				o:AddKey("Formula", 0)
			end
		o:Commit()
	...
	local file = io.open(RmGetString("@") .. "Meters.inc", "w+")
	file:write(oMeters:ToString())
	file:close()

	local file = io.open(RmGetString("@") .. "Measures.inc", "w+")
	file:write(oMeasures:ToString())
	file:close()
